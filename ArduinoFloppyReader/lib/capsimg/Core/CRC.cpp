...
void MakeCRCTable()
{
	...
	// crc32
	for (i=0; i < static_cast<int>(sizeof(crcpoly)/sizeof(UBYTE)); i++)
	...
	for (i=0; i < 256; i++)
{
	...
	// crc-ccitt
	for (i=0, poly=0; i < static_cast<int>(sizeof(crcpoly_ccitt)/sizeof(UBYTE)); i++)
	...
	for (i=0; i < 256; i++)
{
	...
	// crc-ansi
	for (i=0, poly=0; i < static_cast<int>(sizeof(crcpoly_ansi)/sizeof(UBYTE)); i++)
