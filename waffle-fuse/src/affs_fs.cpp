...
static inline uint32_t ublong(const uint8_t* b, size_t o) { return (uint32_t)blong(b,o); }
...
static inline void puts32(uint8_t* b, size_t o, int32_t v) { put32(b,o,(uint32_t)v); }
...
static std::string blk_name(const uint8_t* b) {
    uint8_t len = b[432]; if(len>30) len=30;
...
static void blk_fix(uint8_t* b, size_t csoff=20) {
    uint32_t s=0;
    for(size_t i=0;i<512;i+=4) if(i!=csoff) s+=ublong(b,i);
...
static void blk_set_mtime(uint8_t* b, time_t t=0) {
    ...
    put32(b,420,days); put32(b,424,mins); put32(b,428,ticks);
}
...
static uint32_t affsHash(const char* name, size_t len, uint32_t tableSize) {
    ...
    for(size_t i=0;i<len;i++) h=(h*13+(uint32_t)toupper((unsigned char)name[i]))&0x7FFu;
    return h % tableSize;
}
...
struct AffsFs {
    ...
    bool readBlock(uint32_t n, uint8_t out[512]) const {
        ...
        if(it!=cache.end()){memcpy(out,it->second.data(),512);return true;}
    ...
    int32_t allocBlock() {
        ...
        for(uint32_t n=2;n<numBlocks;n++) {
            ...
            if(isBlkFree(n)) { bitmapMark(n,false); return (int32_t)n; }
        }
        return -1;
    }
    ...
    std::vector<DirEntry> listDir(uint32_t dirBlock) const {
        ...
        for(uint32_t i=0;i<HT_SIZE;i++) {
            ...
            while(blk>0&&(uint32_t)blk<numBlocks) {
                ...
                DirEntry de; de.name=blk_name(eb); de.block=(uint32_t)blk;
                de.isDir=(st==ST_DIR); de.isSoftLink=(st==ST_LNKFILE);
                de.size=(st==ST_FILE)?blk_size(eb):0; de.mtime=blk_mtime(eb);
    ...
    // Path resolution
    struct LookupResult { bool found=false; uint32_t block=0; int32_t secType=0; int32_t size=0; time_t mtime=0; };
    static std::vector<std::string> splitPath(const char* p) {
        std::vector<std::string> parts; std::string cur;
        for(;*p;++p) { if(*p=='/'){if(!cur.empty()){parts.push_back(cur);cur.clear();}} else cur+=*p; }
    ...
    LookupResult lookup(const char* path) const {
        ...
        if(parts.empty()){res.found=true;res.block=rootBlock;res.secType=ST_ROOT;return res;}
        ...
        for(size_t pi=0;pi<parts.size();pi++) {
            ...
            while(blk>0&&(uint32_t)blk<numBlocks) {
                ...
                if(strcasecmp(blk_name(eb).c_str(),name.c_str())==0) {
                    if(pi==parts.size()-1) {
                        res.found=true; res.block=(uint32_t)blk; res.secType=blk_secType(eb);
                        res.size=blk_size(eb); res.mtime=blk_mtime(eb); return res;
                    }
                    int32_t st=blk_secType(eb);
                    if(st!=ST_DIR&&st!=(int32_t)ST_ROOT) return res;
                    cur=(uint32_t)blk; found=true; break;
                }
                blk=blk_nextH(eb);
            }
            if(!found) return res;
    ...
    bool freeFileDataBlks(uint32_t fhb) {
        ...
        for(uint32_t b:dbs) freeBlk(b);
        uint8_t fb[512]={}; if(!readBlock(fhb,fb)) return true;
        int32_t ext=blk_ext(fb);
        std::set<uint32_t> seenExt;
        while(ext>0&&(uint32_t)ext<numBlocks) {
            if(!seenExt.insert((uint32_t)ext).second) break;  // cycle detected
            uint8_t eb[512]={}; if(!readBlock((uint32_t)ext,eb)) break;
            int32_t next=blk_ext(eb); freeBlk((uint32_t)ext); ext=next;
        }
        return true;
    }
    ...
    int writeFileData(uint32_t fhb, const uint8_t* data, uint32_t size) {
        ...
        uint8_t fh[512]={}; if(!readBlock(fhb,fh)) return -EIO;
        freeFileDataBlks(fhb);

        const uint32_t bds=isFFS?512u:488u;
        uint32_t ndb=(size+bds-1)/bds; if(!size) ndb=0;
        ...
        for(uint32_t i=0;i<ndb;i++) {
            ...
            if(blk<0){for(uint32_t j=0;j<i;j++) freeBlk(newdb[j]); return -ENOSPC;}
            newdb[i]=(uint32_t)blk;
        }

        // Write data blocks
        for(uint32_t i=0;i<ndb;i++) {
            uint8_t db[512]={}; uint32_t off=i*bds, len=std::min(bds,size-off);
            if(isFFS) {
        ...
        uint32_t numChunks=(ndb+PTRS-1)/PTRS; if(!ndb) numChunks=0;
        ...
        for(uint32_t i=0;i<c0;i++) put32(fh,24+i*4,newdb[c0-1-i]);
        put32(fh,324,size);
        blk_set_mtime(fh);
        put32(fh,504,0);
        ...
        if(numChunks<=1) {
            ...
            for(uint32_t chunk=1;chunk<numChunks;chunk++) {
                int32_t extBlk=allocBlock(); if(extBlk<0) return -ENOSPC;
                ...
                for(uint32_t i=0;i<cnt;i++) put32(eb,24+i*4,newdb[start+cnt-1-i]);
    ...
    bool addToParent(uint32_t parentBlk, uint32_t entryBlk, const std::string& name) {
        uint8_t pb[512]={}; if(!readBlock(parentBlk,pb)) return false;
        uint32_t h=affsHash(name.c_str(),name.size(),HT_SIZE);
        uint8_t eb[512]={}; if(!readBlock(entryBlk,eb)) return false;
    ...
    bool removeFromParent(uint32_t parentBlk, uint32_t entryBlk, const std::string& name) {
        ...
        while(cur&&cur<numBlocks) {
            ...
            uint8_t eb[512]={}; if(!readBlock(cur,eb)) return false;
            if(cur==entryBlk) {
                uint32_t next=ublong(eb,496);
                if(!prev) { put32(pb,24+h*4,next); blk_set_mtime(pb); writeBlock(parentBlk,pb,20); }
                else {
                    uint8_t prevb[512]={}; readBlock(prev,prevb);
                    put32(prevb,496,next); writeBlock(prev,prevb,20);
                }
                return true;
            }
            prev=cur; cur=ublong(eb,496);
        }
        return false;
    }
    ...
    int32_t doCreateEntry(uint32_t parentBlk, const std::string& name, int32_t sectype) {
        ...
        int32_t blk=allocBlock(); if(blk<0) return -1;
        uint8_t b[512]={};
        ...
        if(!writeBlock((uint32_t)blk,b,20)){freeBlk((uint32_t)blk);return -1;}
        if(!addToParent(parentBlk,(uint32_t)blk,name)){freeBlk((uint32_t)blk);return -1;}
...
static uint32_t parentBlock(AffsFs* fs, const char* path) {
    ...
    std::string pp;
    for(size_t i=0;i<parts.size()-1;i++) pp+="/"+parts[i];
...
static int affs_getattr(const char* path, struct stat* st, struct fuse_file_info*) {
    ...
    AffsFs* fs=getAF(); auto res=fs->lookup(path);
    if(!res.found) return -ENOENT;
    bool rw=!fs->disk->isWriteProtected();
    if(res.secType==ST_FILE) { st->st_mode=S_IFREG|(rw?0644:0444); st->st_nlink=1; st->st_size=res.size; }
    else                     { st->st_mode=S_IFDIR|(rw?0755:0555); st->st_nlink=2; }
...
static int affs_readdir(const char* path, void* buf, fuse_fill_dir_t filler,
    ...
    for(auto& de:fs->listDir(res.block)) {
        ...
        if(de.isDir){s.st_mode=S_IFDIR|(rw?0755:0555);s.st_nlink=2;}
        else        {s.st_mode=S_IFREG|(rw?0644:0444);s.st_nlink=1;s.st_size=de.size;}
...
static int affs_read(const char* path, char* buf, size_t size, off_t offset, struct fuse_file_info*) {
    AffsFs* fs=getAF(); auto res=fs->lookup(path);
    if(!res.found) return -ENOENT; if(res.secType!=ST_FILE) return -EISDIR;
...
static int affs_create(const char* path, mode_t, struct fuse_file_info* fi) {
    AffsFs* fs=getAF(); if(fs->disk->isWriteProtected()) return -EROFS;
    auto parts=AffsFs::splitPath(path); if(parts.empty()) return -EINVAL;
    uint32_t pb=parentBlock(fs,path);
    auto ex=fs->lookup(path);
    if(ex.found){ if(ex.secType!=ST_FILE) return -EISDIR; fi->fh=ex.block; return 0; }
    int32_t blk=fs->doCreateEntry(pb,parts.back(),ST_FILE);
    if(blk<0) return -ENOSPC; fi->fh=(uint64_t)blk; return 0;
}
...
static int affs_unlink(const char* path) {
    ...
    if(res.secType!=ST_FILE) return -EISDIR;
    uint8_t fh[512]={}; if(!fs->readBlock(res.block,fh)) return -EIO;
    uint32_t pb=ublong(fh,500);
    fs->freeFileDataBlks(res.block); fs->freeBlk(res.block);
    auto parts=AffsFs::splitPath(path);
...
static int affs_rmdir(const char* path) {
    ...
    uint8_t db[512]={}; if(!fs->readBlock(res.block,db)) return -EIO;
...
static int affs_rename(const char* from, const char* to, unsigned int flags) {
    ...
    AffsFs* fs=getAF(); if(fs->disk->isWriteProtected()) return -EROFS;
    auto src=fs->lookup(from); if(!src.found) return -ENOENT;
    auto toParts=AffsFs::splitPath(to); if(toParts.empty()) return -EINVAL;
    ...
    if(dst.found) {
        ...
        uint8_t dh[512]={}; fs->readBlock(dst.block,dh);
        uint32_t dp=ublong(dh,500);
        fs->freeFileDataBlks(dst.block); fs->freeBlk(dst.block);
    ...
    uint8_t eb[512]={}; if(!fs->readBlock(src.block,eb)) return -EIO;
    blk_set_name(eb,dstName); put32(eb,500,dstParent); put32(eb,496,0);
