...
static std::string ctlSocketPath(const std::string& portName)
{
    ...
    std::string base =
        (slash != std::string::npos) ? portName.substr(slash + 1) : portName;
...
void usage(const char* progname)
{
    std::cerr
        << "Usage: " << progname
        << " [--verbose] <serial-port> [nbd-address] [nbd-port]\n"
        << "       " << progname
        << " --serve-image <image.adf|img> [nbd-address] [nbd-port]\n"
        << "       " << progname << " --probe <serial-port>\n"
        << "       " << progname
        << " --format <amiga|pc> [--hd] <serial-port>\n"
        << "       " << progname << " --format <amiga|pc> [--hd] <output.adf>\n"
        << "       " << progname << " --ctl <socket> format <amiga|pc> [hd]\n"
        << "       " << progname
        << " --write-image <amiga|pc> [hd] <output.adf>\n"
        << "       " << progname
        << " --write-image <input.adf|img> <serial-port>\n"
        << "\n"
        << "  --verbose         Enable verbose debug logging to stderr.\n"
        << "  --serve-image     Serve an ADF or IMG file as an NBD block "
           "device.\n"
        << "                    No hardware required; writes are flushed back "
           "to the file.\n"
        << "  --probe           Check if a Waffle/DrawBridge device is present "
           "and a disk\n"
        << "                    is loaded (exit 0=disk, 1=no disk, "
           "2=port/hardware error).\n"
        << "  --format amiga    Format with blank Amiga OFS filesystem.\n"
        << "  --format pc       Format with blank PC FAT12 filesystem.\n"
        << "                    Target is a /dev/ttyXXX port (hardware) or a "
           "file (image).\n"
        << "  --hd              (with --format) Use high-density instead of "
           "DD.\n"
        << "  --ctl <socket>    Send a command to a running waffle-nbd "
           "server.\n"
        << "                    socket defaults to /run/waffle-nbd/<port>.ctl\n"
        << "                    Commands:\n"
        << "                      format amiga|pc [hd]  — format inserted "
           "disk\n"
        << "                      clone <output.adf>    — read disk → image "
           "file\n"
        << "                      dump  <input.adf>     — write image file → "
           "disk\n"
        << "  --write-image     amiga|pc [hd] <file>: create a blank image "
           "file.\n"
        << "                    <file> <port>: write an existing ADF/IMG to "
           "hardware\n"
        << "                    (format auto-detected from magic bytes and "
           "file size).\n"
        << "\n"
        << "Example:\n"
        ...
        << "  " << progname
        << " --ctl /run/waffle-nbd/ttyUSB0.ctl format amiga\n"
        << "  " << progname
        << " --ctl /run/waffle-nbd/ttyUSB0.ctl format pc hd\n"
        << "  " << progname
        << " --ctl /run/waffle-nbd/ttyUSB0.ctl clone /tmp/backup.adf\n"
        << "  " << progname
        << " --ctl /run/waffle-nbd/ttyUSB0.ctl dump /tmp/workbench.adf\n"
...
{
    // Strip --verbose / -v anywhere before the mode flag (e.g. --verbose
    // --write-image ...).
    ...
    // --probe <serial-port>: lightweight disk-presence check; no server
    // started. Exit code: 0 = disk present, 1 = no disk, 2 = port/hardware
    // error.
    ...
    // If the target starts with /dev/ it formats the physical floppy via
    // hardware; otherwise the target is treated as a file path and a disk image
    // is written.
    if (argc >= 3 && strcmp(argv[1], "--format") == 0)
    {
        ...
        const char* densName =
            wantsHD ? "HD (1.76 MB)" : "DD (880 KB / 720 KB)";
        ...
        if (target.rfind("/dev/", 0) != 0)
        {
            ...
            for (auto& sec : sectors)
                ofs.write(reinterpret_cast<const char*>(sec.data()),
                          sec.size());
        ...
        std::cout << "waffle-nbd: formatting as " << fmtName
                  << "  density=" << densName << "\n"
                  << "waffle-nbd: writing 160 tracks — this takes about 3-5 "
                     "minutes\n";

        int lastPct = -1;

        // Lambda for formatDisk
        bool ok = disk.formatDisk(
            ftype, wantsHD, [&](int cur, int total, const std::string& msg) {
    ...
    // --write-image <input.adf|img> <serial-port>: write an existing image to
    // hardware.
    // --write-image <amiga|pc> [hd] <output.adf|img>: build a blank image file
    // (no hardware).
    if (argc >= 3 && strcmp(argv[1], "--write-image") == 0)
    {
        const char* arg2 = argv[2];

        // Detect mode: if second arg is "amiga" or "pc" → build blank image to
        // file.
        if (strcmp(arg2, "amiga") == 0 || strcmp(arg2, "pc") == 0)
        {
            ...
            std::cout << "waffle-nbd: building "
                      << (ftype == FormatType::Amiga_OFS ? "Amiga OFS"
                                                         : "PC FAT12")
        ...
        // ───────────────────────────── Usage: --write-image <input.adf|img>
        // <serial-port>
        if (argc < 4)
        {
            usage(argv[0]);
        ...
        if (fileSize % 512 != 0 || fileSize < 512)
        {
            std::cerr << "waffle-nbd: " << imgPath
                      << " is not a valid sector-aligned image (size="
                      << fileSize << ")\n";
            return 1;
        }
        ...
        int lastPct = -1;
        // Lambda for writeDiskImage
        bool ok = disk.writeDiskImage(
            ftype, isHD, sectors,
    ...
    if (strcmp(argv[1], "--serve-image") == 0)
    {
        ...
        int nbdPort = (argc >= 5) ? std::stoi(argv[4]) : findFreePort(nbdAddr);
        if (nbdPort < 0)
        {
        ...
        std::string base =
            (slash != std::string::npos) ? imgPath.substr(slash + 1) : imgPath;
    ...
    int nbdPort = (argc >= 4) ? std::stoi(argv[3]) : findFreePort(nbdAddr);
    if (nbdPort < 0)
    {
        std::cerr << "waffle-nbd: no free TCP port found\n";
        return 1;
