...
NBDServer::NBDServer(std::unique_ptr<IDiskImage> disk) : m_disk(std::move(disk))
{
...
bool NBDServer::run(const std::string& address, int port)
{
    ...
    while (m_running)
    {
        if (!skipToPhase3)
        {
            ...
            while (m_running)
            {
                // This is like the clicking noise as Amiga does (every 2 or 3
                // seconds)
                VLOG("nbd: probing...");
                if (m_disk->probePresent())
                    break;
                // probe() itself takes ~100-500ms; add a small gap to avoid
                // hammering the serial port between retries.
                // We do every 2.5secs as a good compromise
            ...
            {
                std::cerr
                    << "nbd: failed to open port after probe, retrying...\n";
                continue;
            }
        }
        skipToPhase3 = false;
        ...
        while (m_running && !diskGone)
        {
            ...
            while (m_running)
            {
                ...
                if (r > 0)
                {
                    ...
                    clientFd =
                        accept(m_serverFd, (sockaddr*)&clientAddr, &clientLen);
                    if (clientFd != -1)
                        break;
                }
            ...
            else
            {
                ...
                std::cerr
                    << "nbd: format detection failed, retrying in 3s...\n";
                close(clientFd);
                m_disk->closeDisk();
        ...
        // ───────────────────── Port is open, motor is off (parkDisk was called
        // after serve, or disk was removed). checkPresence() uses only
        // COMMAND_CHECKDISKEXISTS. Skip this phase if format/dump just
        // completed via control socket: the disk port is still open and
        // geometry already updated — go straight to Phase 3 to accept the
        // reconnecting nbd-client.
        if (m_skipAbsentWait.exchange(false))
        {
            std::cout << "nbd: skipping disk-absent wait (format/dump just "
                         "completed)\n";
            skipToPhase3 = true;
...
bool NBDServer::handshake(int fd)
{
    ...
    if (!write_all(fd, &init, sizeof(init)))
        return false;

    // 2. Client flags
    uint32_t client_flags_be;

    if (!read_all(fd, &client_flags_be, sizeof(client_flags_be)))
        return false;
    ...
    while (true)
    {
        ...
        if (!read_all(fd, &option, sizeof(option)))
            return false;
        ...
        if (option.len > 0)
        {
            if (!read_all(fd, data.data(), option.len))
                return false;
        }
        ...
        if (option.opt == NBD_OPT_EXPORT_NAME)
        {
            ...
            if (!write_all(fd, &size_be, sizeof(size_be)))
                return false;
            if (!write_all(fd, &flags_be, sizeof(flags_be)))
                return false;

            if (!client_no_zeroes)
            {
                uint8_t zeroes[124] = {0};
                if (!write_all(fd, zeroes, sizeof(zeroes)))
                    return false;
            }
            return true;
        }
        else if (option.opt == NBD_OPT_LIST)
        {
            ...
            if (!write_all(fd, &rep, sizeof(rep)))
                return false;
            if (!write_all(fd, name, nlen))
                return false;
        ...
        {
            std::cout << "nbd: client requested "
                      << (option.opt == NBD_OPT_GO ? "GO" : "INFO") << "\n";

            uint64_t size = (uint64_t)m_disk->totalSectors() * 512;
            uint16_t xflags = NBD_FLAG_HAS_FLAGS | NBD_FLAG_SEND_FLUSH;
            if (m_disk->isWriteProtected())
                xflags |= NBD_FLAG_READ_ONLY;
        ...
        {
            std::cout << "nbd: client requested unsupported option "
                      << option.opt << "\n";
            struct
            {
                uint64_t magic;
        ...
        else if (type == NBD_CMD_WRITE)
        {
            ...
            else
            {
                ...
                for (uint32_t lba = first_lba; lba <= last_lba; ++lba)
                {
                    ...
                    if (sec_len == 512)
                    {
                        if (!m_disk->writeSector(lba, data.data() + buf_pos))
                            ok = false;
                    }
                    else
                    {
                        ...
                        {
                            memcpy(sector + sec_off, data.data() + buf_pos,
                                   sec_len);
                            if (!m_disk->writeSector(lba, sector))
                                ok = false;
                        }
                    }
...
void NBDServer::runControlThread()
{
    ...
    {
        std::cerr << "nbd-ctl: bind(" << m_ctlPath
...
void NBDServer::handleControlClient(int fd)
{
    ...
    if (cmd == "format")
    {
        ...
        // Lambda for formatDisk
        bool ok = m_disk->formatDisk(
            ftype, isHD, [&](int cur, int total, const std::string& msg) {
                int pct = (total > 0) ? (cur * 100 / total) : 100;
                std::string resp = "progress " + std::to_string(pct) + " " +
                                   std::to_string(cur) + " " +
                                   std::to_string(total) + " " + msg + "\n";
        ...
        if (ok)
        {
            ...
            // Allow immediate reconnect without disk removal (geometry
            // changed).
        ...
        {
            const char* err =
                "error: format failed (disk write-protected or removed?)\n";
            write_all(fd, err, strlen(err));
        }
        return;
    }
    ...
    // ── clone <output-file>
    // ─────────────────────────────────────────────────── Reads all sectors
    // from the currently inserted disk and saves to a file.
    if (cmd == "clone")
    {
        if (arg1.empty())
        {
        ...
        for (uint32_t lba = 0; lba < total; lba++)
        {
            ...
            {
                std::string err =
                    "error: read error at sector " + std::to_string(lba) + "\n";
                write_all(fd, err.c_str(), err.size());
                readError = true;
                break;
            }
    ...
    // ── dump <input-file>
    // ───────────────────────────────────────────────────── Writes a disk image
    // file to the currently inserted physical disk. The format is auto-detected
    // from the image (magic bytes + size).
    if (cmd == "dump")
    {
        if (arg1.empty())
        {
        ...
        {
            std::string err =
                "error: " + arg1 + " is not a valid sector-aligned image\n";
            write_all(fd, err.c_str(), err.size());
            return;
        }
        ...
        FormatType ftype = (b0[0] == 'D' && b0[1] == 'O' && b0[2] == 'S')
                               ? FormatType::Amiga_OFS
                               : FormatType::PC_FAT12;
        bool isHD = (numSectors > 1760);

        std::cerr << "nbd-ctl: dump request file=" << arg1
                  << " sectors=" << numSectors << " type="
                  << (ftype == FormatType::Amiga_OFS ? "amiga" : "pc")
                  << " hd=" << (isHD ? "1" : "0") << "\n";

        // Cast to WaffleDisk to call writeDiskImage (it's an extension of
        // IDiskImage).
        ...
        {
            const char* err =
                "error: write failed (disk write-protected or removed?)\n";
            write_all(fd, err, strlen(err));
        }
        return;
    }

    const char* err = "error: unknown command (use: format amiga|pc [hd]  |  "
                      "clone <file>  |  dump <file>)\n";
    write_all(fd, err, strlen(err));
}
