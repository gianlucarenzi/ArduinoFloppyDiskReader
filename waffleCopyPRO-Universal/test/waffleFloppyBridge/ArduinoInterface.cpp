...
DiagnosticResponse ArduinoInterface::readCurrentTrack(void* trackData, const int dataLength, const bool readFromIndexPulse) {
	...
	if (dataLength == RAW_TRACKDATA_LENGTH_DD && m_isHDMode) {
		...
		DBG_E("Error on datalength: %d in HDMODE\n", dataLength);
		return m_lastError;
	}
	if (dataLength == RAW_TRACKDATA_LENGTH_HD && !m_isHDMode) {
		m_lastError = DiagnosticResponse::drMediaTypeMismatch;
		DBG_E("Error on datalength: %d in DDMODE\n", dataLength);
		return m_lastError;
	}
	...
	if (m_isHDMode) {
		...
		DBG_N("COMMAND_READTRACK (1) DONE\n");

		// Allow command retry
		if (m_lastError != DiagnosticResponse::drOK) {
			DBG_E("ERROR ON READTRACK (DD) Clearing all buffer...\n");
			// Clear the buffer
			bool hasRead = deviceRead(tmp, dataLength);
			DBG_N("deviceRead returns: %d\n", hasRead);
			fflush(stderr);
			m_lastError = runCommand(COMMAND_READTRACK);
			if (m_lastError != DiagnosticResponse::drOK) {
				free(tmp);
				DBG_E("No COMMAND_READTRACK in DD!!!\n");
				return m_lastError;
			}
			DBG_N("COMMAND_READTRACK RETRY GOOD\n");
		}
		...
		int bytePos = 0;
		int readFail = 0;
		DBG_N("Keep reading until he hit dataLength or a null byte is received\n");
		for (;;) {
			unsigned char value;
			DBG_N("Calling deviceRead(), readFail: %d\n", readFail);
...
DiagnosticResponse ArduinoInterface::runCommand(const char command, const char parameter, char* actualResponse) {
	...
	if (parameter != '\0')
		DBG_N("Has Parameter: %d - '%c'\n", parameter, parameter);
		if (!deviceWrite(&parameter, 1)) {
			m_lastError = DiagnosticResponse::drSendParameterFailed;
			DBG_E("Send Command Parameter %d error\n", parameter);
			return m_lastError;
		}

	bool resp = deviceRead(&response, 1, true);
	DBG_N("COMMAND READ: %d\n", resp);
...
bool ArduinoInterface::deviceRead(void* target, const unsigned int numBytes, const bool failIfNotAllRead) {
	DBG_N("Called\n");
	if (!m_comPort.isPortOpen()) {
		DBG_E("COMPORT is not open\n");
		return false;
	}

	DBG_N("Now Reading %d and failIfNotAllRead: %d...\n", numBytes, failIfNotAllRead);
	unsigned long read = m_comPort.read(target, numBytes);

	DBG_N("Read %ld -- NumBytes: %d...\n", read, numBytes);
	if (read < numBytes) {
		if (failIfNotAllRead) {
			DBG_E("failIfNotAllRead\n");
			return false;
		}
...
bool ArduinoInterface::deviceWrite(const void* source, const unsigned int numBytes) {
	DBG_N("Called: numBytes: %ld\n", numBytes);
	bool rval = m_comPort.write(source, numBytes) == numBytes;
	DBG_N("Exit with: %d\n", rval);
	return rval;
}
