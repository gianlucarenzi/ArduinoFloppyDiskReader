...
bool SerialIO::isPortOpen() const {
	bool rval = false;
#ifdef FTDI_D2XX_AVAILABLE
	if (m_ftdi.isOpen()) rval = true;
#endif
#ifdef _WIN32
	rval = m_portHandle != INVALID_HANDLE_VALUE;
#else
	rval = m_portHandle != -1;
#endif
	DBG_N("Rval %d\n", rval);
...
void SerialIO::purgeBuffers() {
	if (!isPortOpen()) {
		DBG_E("Port is not opened\n");
		return;
	}
#ifdef FTDI_D2XX_AVAILABLE
	if (m_ftdi.isOpen()) {
		DBG_N("FTDI_D2XX Port is Open, now Purge\n");
...
void SerialIO::setBufferSizes(const unsigned int rxSize, const unsigned int txSize) {
	...
	if (m_ftdi.isOpen()) {
		...
		DBG_N("FTDI_D2XX SetDevice Buffer RX SIZE: %d - TX SIZE: %d\n", rxSize < 256 ? rxSize : 256, txSize); 
		return;
	}
#endif

	DBG_N("SetDevice Buffer RX SIZE: %d - TX SIZE: %d\n", rxSize, txSize); 
#ifdef _WIN32
...
SerialIO::Response SerialIO::openPort(const std::wstring& portName) {
	...
	updateTimeouts();
	DBG_N("EveryThing is OK\n");
	return Response::rOK;
#endif

	DBG_E("NotImplemented\n");
...
SerialIO::Response SerialIO::configurePort(const Configuration& configuration) {
...
#ifdef FTDI_D2XX_AVAILABLE
	if (m_ftdi.isOpen()) {
		DBG_N("FTDI_2XX Configuration\n");
		if (m_ftdi.FT_SetFlowControl(configuration.ctsFlowControl ? FT_FLOW_RTS_CTS : FT_FLOW_NONE, 0, 0) != FTDI::FT_STATUS::FT_OK) {
			DBG_E("FTDI_2XX Error on SetFlowControl\n");
			return SerialIO::Response::rUnknownError;
		}
		if (m_ftdi.FT_SetDataCharacteristics(FTDI::FT_BITS::_8, FTDI::FT_STOP_BITS::_1, FTDI::FT_PARITY::NONE) != FTDI::FT_STATUS::FT_OK) {
			DBG_E("FTDI_2XX Error on SetDataCharacteristic\n");
			return SerialIO::Response::rUnknownError;
		}
		if (m_ftdi.FT_SetBaudRate(configuration.baudRate) != FTDI::FT_STATUS::FT_OK) {
			DBG_E("FTDI_2XX Error SetBaudRate\n");
			return SerialIO::Response::rUnknownError;
		}
		m_ftdi.FT_SetLatencyTimer(2);
		m_ftdi.FT_ClrDtr();
		m_ftdi.FT_ClrRts();
		DBG_N("FTDI_2XX Configuration OK\n");
		return SerialIO::Response::rOK;
	}
#endif

	DBG_N("Configuration Setup\n");
#ifdef _WIN32
	// Configure the port
	COMMCONFIG config;
	...
	config.dcb.fInX = 0;
	srConfig = SetCommConfig(m_portHandle, &config, sizeof(config)) ? Response::rOK : Response::rUnknownError;
	DBG_N("Configuration %d\n", srConfig);
#else
	if (tcgetattr(m_portHandle, &term) < 0) {
		DBG_E("TERMIOS GETATTR\n");
		return Response::rUnknownError;
	}
#ifdef cfmakeraw
	cfmakeraw(&term);
	...
	if (ioctl(m_portHandle, TCSETS2, &term) < 0) {
		DBG_E("TERMIOS TCSETS2\n");
		return Response::rUnknownError;
	}
#else
	term.c_cflag &= ~CBAUD;
	...
	if (tcsetattr(m_portHandle, TCSANOW, &term) != 0) {
		DBG_E("TERMIOS TCSANOW\n");
		return Response::rUnknownError;
	}
	...
	DBG_N("Configuration OK\n");
	return Response::rOK;
#endif

	DBG_E("Configuration NOTIMPLEMENTED\n");
	return Response::rNotImplemented;
}
...
bool SerialIO::checkForOverrun() {
	...
	if (m_ftdi.isOpen()) {
		ULONG status;
		DBG_N("FTDI_D2XX GetModemStatus\n");
		if (m_ftdi.FT_GetModemStatus(&status) != FTDI::FT_STATUS::FT_OK) {
			DBG_E("FTDI_D2XX GetModemStatus\n");
			return false;
		}
		rval = (status & (FT_MODEM_STATUS_OE | FT_MODEM_STATUS_FE)) != 0;
		DBG_N("FTDI_D2XX GetModemStatus %d\n", rval);
		return rval;
	}
#endif
	...
	if (!ClearCommError(m_portHandle, &errors, &comstatbuffer)) {
		DBG_E("Unable to ClearCommError()\n");
		return 0;
	}
	rval = (errors & (CE_OVERRUN | CE_FRAME | CE_RXOVER)) != 0;
	DBG_N("WIN32 rval: %d\n", rval);
#else
	rval = false;
	DBG_N("POSIX rval: %d\n", rval);
...
unsigned int SerialIO::getBytesWaiting() {
	...
	if (m_ftdi.isOpen()) {
		DWORD queueSize = 0;
		if (m_ftdi.FT_GetQueueStatus(&queueSize) != FTDI::FT_STATUS::FT_OK) {
			DBG_E("FTDI_2XX GetQueueStatus\n");
			return 0;
		}
		DBG_N("FTDI_2XX GetQueueSize: %u\n", queueSize);
		return queueSize;
	}
#endif
	...
	if (!ClearCommError(m_portHandle, &errors, &comstatbuffer)) {
		DBG_E("ClearCommError()\n");
		return 0;
	}
	DBG_N("ComStatBuffer.cbInQue: %u\n", comstatbuffer.cbInQue);
	return comstatbuffer.cbInQue;
#else
	int waiting;
	if (ioctl(m_portHandle, TIOCINQ, &waiting) < 0) {
		DBG_E("TERMIOS TIOCINQ\n");
		return 0;
	}
	DBG_N("TERMIOS TIOCINQ %u\n", (unsigned int)waiting);
...
unsigned int SerialIO::write(const void* data, unsigned int dataLength) {
	if ((data == nullptr) || (dataLength == 0)) {
		DBG_E("data == nullptr or dataLength == 0\n");
		return 0;
	}
	if (!isPortOpen()) {
		DBG_E("Port not opened\n");
...
void SerialIO::setReadTimeouts(unsigned int waitTimetimeout, unsigned int multiplier) {
	...
	DBG_N("Called m_readTimeout %d - m_readTimeoutMultiplier %d\n", m_readTimeout, m_readTimeoutMultiplier);
	updateTimeouts();
	DBG_N("Exit\n");
}
...
void SerialIO::setWriteTimeouts(unsigned int waitTimetimeout, unsigned int multiplier) {
	...
	DBG_N("Called m_writeTimeout %d, m_writeTimeoutMultiplier %d\n", m_writeTimeout, m_writeTimeoutMultiplier);
	updateTimeouts();
	DBG_N("Exit\n");
}
